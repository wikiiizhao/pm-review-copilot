...
| 能力 | 说明 |
| --- | --- |
| 项目记忆初始化 | 创建 `project-memory/`，包含当前事实、决策、证据、假设、审查日志、标注审查、用户偏好和恢复记录。 |
| 记忆生命周期管理 | 将记忆标记为 `pinned`、`active`、`background`、`archived`、`superseded`、`deprecated` 或 `needs_review`，让过时内容默认不再占用 Agent 上下文。 |
...
```text
使用 $pm-review-copilot-zh 审查这份 PRD。请重点找出无证据主张、指标口径问题、与 project-memory 冲突的地方，以及需要我人工确认的内容。
```
...
```text
使用 $pm-review-copilot-zh 把今天讨论的新决策、证据和开放问题更新到 project-memory。
```
