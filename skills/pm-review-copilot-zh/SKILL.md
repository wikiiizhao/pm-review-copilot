...
description: 面向产品经理的项目记忆、手动更新和审查关卡工作流，适用于 Agent 辅助生成的 PRD、策略文档、数据洞察、长方案和标注任务。Use when the user explicitly asks in Chinese or requests Chinese output to initialize or update durable project memory, resume a PM project from memory, review long Agent outputs for hallucinations or unsupported claims, compare revised PRD/strategy drafts, identify human-review hotspots, or audit labeling consistency. 所有面向用户的总结、审查结论、模板和脚本报告都应使用中文输出。
---
...
这个 skill 不会自动触发。它不会监听文件、定时运行，也不会在每次 Agent 回复后自行审查。只有在用户明确要求初始化记忆、更新记忆、从记忆恢复项目、审查文档或审计标注时才使用。
...
优先在项目或文档工作区中使用 `pm-memory/` 目录。如果会和用户已有文件冲突，改用 `project-memory/`，并在 `CONTEXT_MANIFEST.md` 中记录实际位置。

默认文件：

- `CONTEXT_MANIFEST.md`：记忆文件地图、读取顺序、事实来源规则、忽略规则。
...
- `LABEL_AUDIT.md`：标注一致性审计和标签定义问题。
- `MEMORY_UPDATE_LOG.md`：手动记忆刷新记录、处理来源、变更文件、跳过项。
...
5. `CURRENT_STATE.md`
6. `DECISION_LOG.md` 中最新且相关的条目
7. `EVIDENCE_LOG.md` 中最新且相关的条目
8. `HYPOTHESIS_LAB.md` 中相关假设
9. 最新且相关的 `REVIEW_LOG.md`、`LABEL_AUDIT.md` 或 `MEMORY_UPDATE_LOG.md`
...
当用户说“更新 memory”“刷新 pm-memory”“同步今天的信息”“把新内容更新到项目 memory”等类似请求时，使用这个工作流。该流程是手动、由用户触发的。

1. 读取 `CONTEXT_MANIFEST.md`、`USER_PREFERENCES.md`、`HUMAN_BRIEF.md`、`CURRENT_STATE.md`，以及与新材料相关的日志。
...
- `scripts/init_pm_memory.py`：创建中文 PM 记忆文件夹和启动文件。
- `scripts/label_consistency_audit.py`：审计 CSV 中重复标注的一致性，并输出中文 Markdown 报告。
- `scripts/doc_diff_review.py`：比较两个草稿，生成中文 Markdown diff 报告。

## 参考文件

需要精确文件模板、审查报告 schema 或来源设计说明时，读取 `references/pm-memory-templates.md`。
